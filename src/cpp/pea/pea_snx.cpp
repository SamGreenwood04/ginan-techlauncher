...
void sinexPerEpochPerStation(Trace& trace, GTime time, Receiver& rec)
{
    ...
    {
        if (rec.antDelta.isZero() &&
            rec.snx.ecc_ptr != nullptr)  // todo Eugene: ecc_ptr/ant_ptr/rec_ptr will never be
                                         // nullptr as they are initialised with dummy variables -
                                         // so 0/empty values can still be assigned!
        {
