...
)
{
    ...
    if ((acsConfig.process_preprocessor == true &&
         (acsConfig.preprocOpts.preprocess_all_data == realEpoch)
        ) ||  // Only preprocess once either when reading data or in main processing
        (acsConfig.process_preprocessor == false && realEpoch == false
        ))  // Still do basic preparation despite preprocessor (only skip when the data falls out of
            // processing epochs)
    {
        return;
    }
